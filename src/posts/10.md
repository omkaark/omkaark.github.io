...
I want to support a 4096 context length but with the available compute and given attention is really quadratic during training, it would make more sense to keep context shorter and increase batches. Generally, labs train on shorter context for 80-90% of the run and then, go ham in the last 10-20%.

Another reason is in most datasets, 80-90% of the rows are < 2k tokens, so generally, it's not like we have nice long samples to train on anyway. Therefore, I will train on 2048 as my sequence length. I also omit cross-document masking for now for simplicity's sake.

For those unfamiliar, this is what the architecture we are training looks like:
![Llama-3 Architecture](/Users/omkaarwork/Desktop/projects/personal-website/public/4-lfm/llama.png)
...
class ModelConfig:
    ...
    norm_eps: float = 1e-5
    max_position_embeddings: int = 2**12
...
Now, let's estimate memory usage: total FP32 memory = (activations + weights + gradients + optimizer state) + misc. 
...
Activations = Dependant on implementation. Let's say n * 1241581568, where n = batch size of inputs in [n, 2048]. Empirically on a non-compiled model, I get n around 1.6 for [1, 2048] input. It scales linearly (I confirmed this empirically with an experiment) with batch size = 7.95GB
...
Since torch does not know the computational graph (without compiling), it saves each op's result as an activation for the backward pass, resulting in more memory reserved than needed. Thankfully, there is a known easy path to reducing activation memory, torch.compile, and any unoptimized corner of the profile can be fixed using CUDA kernels. Both try to fuse these ops which eliminates most intermediate activations!

Let’s talk about token budget (total tokens to train). Scaling laws say a [1:20](https://medium.com/@aiml_58187/beyond-bigger-models-the-evolution-of-language-model-scaling-laws-d4bc974d3876) ratio of params:tokens is optimal. I have 1B params, so 20B tokens is what I need to train on. My target batch size is 1M (2^20) tokens which I chose because GPT-3 XL was trained on 1M (2^20) batch size. ![GPT Params vs Batch Size Table](https://developer-blogs.nvidia.com/wp-content/uploads/2023/03/OpenAI-GPT-3.png)

20B tokens / 1M tokens = 20000 training steps. Now, let’s do some back of the envelope maths with the naive non-compiled numbers above, I have 8xH100’s meaning 80gb per gpu. Training through [1, 2048] tokens takes (4 + 1.6) * 1241581568 * 4 in FP32 memory which is 25.9GB of memory maximum. [N, 2048] tokens should take (4 + N * 1.6) * 1241581568 * 4 in FP32 memory which is 19.8GB + N * 7.95GB. On one 80GB H100 (accounting 5GB misc for kernels, etc), ~7 batches of 2048 tokens should fit given my basic calculations!
...
Remember this is without any optimizations. But, I have a few ideas up my sleeve which don't affect accuracy: torch compile, flashattn, gradient accumulation, mixed precision w/ BF16, etc. The plan is to go with DDP where each GPU keeps a copy of one model, runs one train step and syncs (averages) gradients before applying it to all the GPUs. Worst case, we can even move to FSDP2 which shards parameters, grads and optimizer states to reduce base memory per GPU, however this adds a communications overhead (we might be able to do something about it).

### Hitting a reproducibility snag
I have been calculating activation memory by measuring allocation on my mac. When I ran these on 1xH100, the activation memory on a [1, 2048] tokens input is astronomically more. My mac shows 6.2GB while the H100 shows 22.5GB?? 

My first thought is could H100 have more ops which means more activations are saved? This doesn't make sense however, ops are the same when using no computational graphs (no torch.compile). To debug this I spoke with my confidants claude and chadgpt. 

Claude was more helpful and was a better brainstorming partner. After brainstorming debugging ideas and spending time with my debugger to check ideas off, we cowrote a script which calculated the memory diff as we forward through each layer and there lied the smoking gun: H100 attention activations per layer was OOMs more compared to CPU attention activations. Upon deeper inspection turns out this was because CUDA SDPA with FP32 inputs falls back to naive attention (MATH backend) which materializes the full attention logit matrix. I confirmed this by manually casting QKV to bf16 and re-casting the output to fp32 to see the same activation memory allocation.
...
Let me explain quickly how this works. Normally, when running pytorch, the operations are simply run in the order they are invoked. On a GPU, these operations invoke a CUDA kernel, and there's all this overhead when invoking CUDA kernels one at a time. On a profiling chart, these look like empty spaces between each kernel running (referred to as "bubbles"). [ADD PIC OF BUBBLES]

Torch compile tries to create a computational graph and fuse most of ops into bigger kernels to eliminate launch overhead and therefore, bubbles. Torch compile also ends up writes there kernels using Triton which means memory access is vastly improved and everything runs fast!

### Mixed Precision
I haven't mastered the dark arts of low-precision training yet. H100's have support for mx/fp8 but I will stick to bf16 mixed-precision for now. Explicit casting means setting the whole model to be the target type (bf16 in our case) which also sets the optimizer states to bf16. BF16 has the same range but lower precision compared to fp32 which could lead to convergence issues due to numeric over/underflow. Therefore, we will use torch's autocast which stores master weights and optimizer states in fp32, but selectively & automatically casts select ops (like attention) to bf16 for forward and backward passes which means a lot of activations are stored in bf16 (max 50% savings in activation memory).
...
There is a good way and a lazy way to do this. Good way is to calculate memory and compute per op and evaluate the tradeoff or look at existing research. Lazy way is what I am about to do... which is bruteforce.
...
What if we checkpoint all layers?
```
...
Which is an 80% increase in compute time compared to no checkpointing for a 44GB drop in reserved memory. Let's be cheeky and try [64, 2048] as inputs:
```
...
I tried switching to allowing the [tf32](https://blogs.nvidia.com/blog/tensorfloat-32-precision-format/) which stores everything in fp32 as we currently have it but does all the math with 19 bit floats (e8m10) on special tf32 compute units which should be faster. However, I did not notice any improvements in speed, so I ommited this. FYI, I simply added `torch.set_float32_matmul_precision("high")` at the top, maybe I missed something?

Pytorch uses Flash Attention 2 (FA-2) which is fast but [Flash Attention 3](https://pytorch.org/blog/flashattention-3/) is 50% faster on the H100 than FA-2. The only problem here is FA-3 needs to be compiled from source on my system and is infamous for it's loooong compile times. My [process](https://pypi.org/project/flash-attn/2.8.3/) to get FA-3 is:
...
4. Replace flash_attn_interface.flash_attn_fun() with SDPA

Once FA-3 compiled, I tried it out in my code but I get an OOM. FA-3 introduces a graph break in my LLM module which could add some inefficiency but not this bad. I reduced batch size to 8 and noticed that it takes more time per step and memory than SDPA's FA-2 implementation. My conclusion is I messed up somewhere in my setup, something related to autocast or checkpointing that probably does not gel well with FA-3. I will revisit this in my next blog and try to get it to work (it would be a big win).

I think checkpointing every thing is not the best for my MFU, so I'd like to revisit my checkpointing scheme in the next blog as well.
...
Since out endstate is running distributed training, there is a question of communication overhead. GPUs need to sync gradients and optimizer states with each other after every backward pass and optimizer step respectively. Given we only distirbuted bytes intra-node and intra-node bandwidth is pretty high, the overhead is a bit less relevant to us. In the next blog, I will showcase this more and look into overlapping / fusing so DDP will have minimal overhead.
...
I expected to face some turbulence during training. I have read multiple great blogs about training convergence issues like the famous [Marin 32B's loss spikes](https://wandb.ai/marin-community/marin/reports/Marin-32B-Spike-Fixing--VmlldzoxMzM1Mzk1NQ) blog. I do think convergence issues are more pronounced in larger training runs, so hopefully we do not run into them.

Because training on 8xH100's directly without verifying every line of code works is a catastrophical waste of money, I rolled out this run in 3 stages. First stage is local device testing where I ran training on my Macbook on a much smaller model. This was to identify any general pytorch related bugs that I might have introduced. Once I solved those, I ran it on 1xH100 where I squashed a few minor torch.distributed cleanups and cuda-related errors. I noticed over 250 steps that my loss was down and to the right which is a good sign. The loss starts of as ~12. Since the model has random weights and no knowledge at the beginning, the probability of getting any token is 1/vocab_size and therefore, cross entropy loss is -ln(1/131072) = 11.78, so we are bang on the expected first loss value. 
...
A problem I had was around 15k steps my loss was stagnant. My intuition was that the learning rate is too low, there was no way for me to verify this given I was not logging things like ||update|| / ||param|| which would have helped me debug this more. So, I decided to stop training, load back the 15k checkpoint with a different scheduler (1.5*base_lr, very low warmup steps, higher minimum lr ratio) on 5000 steps. To no avail, I could not revive the run, so I decided to save some money and leave things as is at 15k steps.

Doing this motion allowed me to verify me checkpointing logic (although I should have tested it on 1xH100 first) and fix all the minor bugs. Major bugs means all the 10+ hours of 8xH100 money is wasted. I don't enjoy YOLO runs and this one did not work.

This is my graph for the first 15,000 steps:

!(15k steps of training a 1B model)[/Users/omkaarwork/Desktop/projects/personal-website/public/x-worklog-1b-llama/8xh100_run.png]

Overall, this was fun, however my MFU is at 18% & train throughput was at 400k tok/s which is no bueno. I know my checkpointing on every layer impacted throughput a lot, however it allows me to skip gradient accumulation, so I need to think a bit more about this tradeoff. 

I plan to train a lot more types of models in the future. I want to minimize costs and inefficiency as much as possible while making my performance gains translatable to all kinds of future runs. I will work on this over next post.
...
It's much easier to iterate from bad to good then land up on good straight away. Don't get me wrong my repo is still far from SOTA but it provides a clear working implementation which I can abstract further later. That brings us to my second reflection, I had to rely on other's observations for learning rate, weight inits, scheduler and other hyperparams. Great researchers have a mix of empirical evidence along with mathematical intution for what works based on 100s of past runs. It's something that I'll just have to iterate and read more papers on.

### Extras

Tim Dettmers has a library called bitsandbytes which is widely used for quantization. They have an nf4 format which is widely used in QLoRA finetuning of models, it stores all parameters in 4-bit int and uses 2 block scaling factors to dequantize them. Check this [paper](https://arxiv.org/abs/2305.14314) to learn more. Anyway, they have an AdamW8bit optimizer which is commonly used in finetuning. 

If training on this over 8k+ steps converges, I can basically get away with using this 8bit version! So I brought it into my 1xH100 training run along with StableEmbedding which is the recommended setting. My reserved memory dropped by 10GB and these are my wandb runs 8bit vs fp32 AdamW:
![NF4 vs FP32 AdamW](/Users/omkaarwork/Desktop/projects/personal-website/public/10-1b-model-p1/nf4-training.png)

At first glance, it seems like it converged even better than the fp32 variant. However, if you look carefully, you will notice my blunder. I messed up and gave 8bit AdamW 3x the peak learning rate. Would I scrap all my results though? No, the claim that AdamW8bit > AdamW32bit is falsifiable but I think AdamW8bit might be right up there with the 32-bit one. I am planning to use it in my next run as it reduces memory and also converges.
